...
function _getRiksdagPartyMap(year) {
  const riksdagParties = JSON.parse(
    fs.readFileSync(path.resolve(__dirname, 'val', String(year), 'partideltagande/riksdag.json'))
      .toString('utf8')
  );
...
// all parties
const parties = loadJSONFile('partier.json');
...
// all regions
const regions = loadJSONFile('regioner.json');
